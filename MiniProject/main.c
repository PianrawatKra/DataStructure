    ...
    void maxHeapify(struct PriorityQueue* pq, int i) {
        int largest = i;    // ตำเเหน่งรากคือตัวที่ใหญ่ที่สุด
        int left = (2 * i) + 1; // หาเด็กเปรต1
        int right = (2 * i) + 2; // หาเด็กเปรต2
        ...
        if (right < pq->size && pq->heap[right] > pq->heap[largest])//เปรียบเทียบโดยชี้ไปยังheap ด้วยพอยเตอร์
            largest = right;
    ...
    int main() {
        ...
        buildMaxHeap(&pq);
        printf("Initial ");
        printPriorityQueue(&pq);//เเสดงค่าในpqชี้ไป
        int y =pq.size;
